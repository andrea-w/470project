...
def set_up_roulette_wheel(candidates_df, accuracy_df):
    ...
    list_of_children = []
    # initialize empty Dataframe of children
    children_candidates_df = pd.DataFrame()
    ...
    children_candidates_df = children_candidates_df.T
    children_candidates_df = children_candidates_df.reindex(range(1, config.NUM_CANDIDATES_PER_GENERATION + 1))
...
def roulette_wheel_selection(candidates_df, prob_cutoffs_array):
    ...
    for x in range(len(prob_cutoffs_array)):
        if (prev_val < rand_float_2) and (prob_cutoffs_array[x] > rand_float_2):
            ...
            prev_val = prob_cutoffs_array[x]
    child_candidate = create_offspring(candidates_df.loc[str(parent1+1),:], candidates_df.loc[str(parent2+1),:])
